...
protected:
    ...
    std::unordered_map<entt::id_type, engine::render::Sprite> sprites_;   ///< @brief 图片集合
    std::unordered_map<entt::id_type, entt::id_type> sounds_;           ///< @brief 音效集合，key为音效名称ID，value为音效ID
    entt::id_type current_sprite_id_ = entt::null;           ///< @brief 当前显示的图片ID
...
public:
    ...
    void addSound(entt::id_type name_id, entt::hashed_string hashd_path);                        ///< @brief 添加音效
